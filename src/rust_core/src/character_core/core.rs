...
use crate::collision::{Collision, CollisionVTable};
...
pub struct CharacterCore {
    ...
    /// Owned collision instance with vtable pointing to C++ CCollision.
    collision: Collision,
...
impl CharacterCore {
    ...
    /// Initialize with a shared WorldCore pointer and optional C++ collision vtable.
    pub fn init(&mut self, world: *mut WorldCore, vtable: Option<CollisionVTable>) {
        self.world = world;
        if let Some(vt) = vtable {
            self.collision.set_vtable(vt);
        }
    }
    ...
    pub fn tick(&mut self, use_input: bool) {
        ...
        let grounded =
            self.collision.check_point(self.pos.x + PHYS_SIZE / 2.0, self.pos.y + PHYS_SIZE / 2.0 + 5.0)
            || self.collision.check_point(self.pos.x - PHYS_SIZE / 2.0, self.pos.y + PHYS_SIZE / 2.0 + 5.0);
        ...
        if self.hook_state == HOOK_IDLE {
            ...
            let (hit_ground, hit_nohook, clipped_pos) = self.collision.intersect_line(self.hook_pos, new_pos);
            new_pos = clipped_pos;
            if hit_ground { going_to_hit_ground = true; }
            else if hit_nohook { going_to_retract = true; }

            // Player hook detection has priority over tile collision
            if world.tuning.player_hooking {
                let mut best_dist = f32::MAX;
                unsafe {
                    for i in 0..MAX_CLIENTS {
                        let pchar = world.ap_characters[i];
                        if pchar.is_null() || pchar == self as *mut _ { continue; }
                        let pchar_ref = &*pchar;
        ...
        if self.hook_state == HOOK_GRABBED {
            ...
            if self.hooked_player == -1 && (self.hook_pos - self.pos).length() > 46.0 {
                ...
                if hook_vel.y > 0.0 { hook_vel.y *= 0.3; }
        ...
        // ---- Player-player collision and hook drag ----
        unsafe {
            for i in 0..MAX_CLIENTS {
                let pchar = world.ap_characters[i];
                if pchar.is_null() || pchar == self as *mut _ { continue; }
    ...
    pub fn move_core(&mut self) {
        ...
        if world.tuning.player_collision {
            ...
            let mut collision_pos = self.pos;
